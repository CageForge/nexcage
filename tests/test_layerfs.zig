...
test "LayerFS dependency ordering" {
    ...
    // Create base layer
    const base_layer = try Layer.createLayer(
        allocator,
        "application/vnd.oci.image.layer.v1.tar",
        "sha256:base1234567890",
        1024,
        null
    );
    ...
    const app_layer = try Layer.createLayer(
        allocator,
        "application/vnd.oci.image.layer.v1.tar",
        "sha256:app1234567890",
        2048,
        null
    );
...
test "LayerFS error handling" {
    ...
    try testing.expectError(image.LayerFSError.LayerNotFound, 
        layerfs.createMountPoint("sha256:nonexistent", "/mnt/test"));
    
    // Test mounting overlay for non-existent layer
    try testing.expectError(image.LayerFSError.LayerNotFound, 
        layerfs.mountOverlay("sha256:nonexistent", "/mnt/overlay"));
