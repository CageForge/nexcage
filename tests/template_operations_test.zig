...
test "detectOS positive/negative" {
    ...
    var tmp = try std.testing.tmpDir(.{}); defer tmp.cleanup();
    const root = tmp.dir;
    try root.makePath("etc");
    var f = try root.createFile("etc/os-release", .{}); defer f.close();
    try f.writeAll("ID=debian\n");
    const p = try std.fmt.allocPrint(A, "{s}", .{tmp.path}); defer A.free(p);
    const os_ok = try template_ops.detectOS(p, A); defer A.free(os_ok);
    try std.testing.expectEqualStrings("debian", os_ok);
    const os_unknown = try template_ops.detectOS("/non/existent", A); defer A.free(os_unknown);
...
test "detectArchitecture positive/negative" {
    ...
    var f = try root.createFile("proc/version", .{}); defer f.close();
    try f.writeAll("Linux version x86_64\n");
    const p = try std.fmt.allocPrint(A, "{s}", .{tmp.path}); defer A.free(p);
    const arch_ok = try template_ops.detectArchitecture(p, A); defer A.free(arch_ok);
    try std.testing.expectEqualStrings("amd64", arch_ok);
    const arch_unknown = try template_ops.detectArchitecture("/non/existent", A); defer A.free(arch_unknown);
...
test "detectVersion positive/negative" {
    ...
    const p = try std.fmt.allocPrint(A, "{s}", .{tmp.path}); defer A.free(p);
    const ver_ok = try template_ops.detectVersion(p, A); defer A.free(ver_ok);
    try std.testing.expectEqualStrings("12", ver_ok);
    const ver_unknown = try template_ops.detectVersion("/non/existent", A); defer A.free(ver_unknown);
...
test "buildMultipartForm positive" {
    ...
    defer A.free(mf.body); defer A.free(mf.content_type);
