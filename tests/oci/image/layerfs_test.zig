...
test "LayerFS add and get layer" {
    ...
    const layer = try Layer.createLayer(allocator, "application/vnd.oci.image.layer.v1.tar", createTestDigest(), 1024, null);
...
test "LayerFS get layers in order" {
    ...
    const layer1 = try Layer.createLayer(allocator, "application/vnd.oci.image.layer.v1.tar", "sha256:layer1abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890", 1024, null);
    defer layer1.deinit(allocator);

    const layer2 = try Layer.createLayer(allocator, "application/vnd.oci.image.layer.v1.tar", "sha256:layer2abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890", 2048, null);
...
test "LayerFS error cases" {
    ...
    try testing.expectError(LayerFSError.LayerNotFound, layerfs.createMountPoint("sha256:nonexistent", "/tmp/test"));

    // Try to mount overlay for non-existent layer
    try testing.expectError(LayerFSError.LayerNotFound, layerfs.mountOverlay("sha256:nonexistent", "/tmp/overlay"));
...
test "LayerFS duplicate operations" {
    ...
    try testing.expectError(LayerFSError.InvalidMountPoint, layerfs.createMountPoint(createTestDigest(), "/tmp/test2"));

    // Mount overlay
    try layerfs.mountOverlay(createTestDigest(), "/tmp/overlay");

    // Try to mount overlay again
    try testing.expectError(LayerFSError.InvalidOverlay, layerfs.mountOverlay(createTestDigest(), "/tmp/overlay2"));
}
...
test "LayerFS memory management" {
    ...
    while (i < 10) : (i += 1) {
        ...
        const layer = try Layer.createLayer(allocator, "application/vnd.oci.image.layer.v1.tar", &digest, @as(u64, i) * 1024, null);
        defer layer.deinit(allocator);
...
test "LayerFS garbage collection" {
    ...
    const layer3 = try Layer.createLayer(allocator, "application/vnd.oci.image.layer.v1.tar", "sha256:layer3abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890", 4096, null);
