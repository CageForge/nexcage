...
pub const StopCommand = struct {
    ...
    pub fn execute(self: *Self, options: core.types.RuntimeOptions, allocator: std.mem.Allocator) !void {
        ...
        if (options.container_id == null) {
            if (self.logger) |log| {
                try log.@"error"("Container ID is required for stop command", .{});
            }
        ...
        switch (runtime_type) {
            .lxc => {
                ...
                try lxc_driver.stop(container_id);
            ...
            .proxmox_lxc => {
                ...
                try proxmox_lxc.stopContainer(vmid);
                
                if (self.logger) |log| {
                    try log.info("Proxmox LXC container stopped successfully: {s} (VMID: {d})", .{ container_id, vmid });
                }
            },
            .proxmox_vm => {
                ...
                try proxmox_vm.stopVm(vmid);
                
                if (self.logger) |log| {
                    try log.info("Proxmox VM stopped successfully: {s} (VMID: {d})", .{ container_id, vmid });
                }
            },
            .crun => {
                ...
                try crun_driver.stopContainer(container_id);
                
                if (self.logger) |log| {
                    try log.info("Crun container stopped successfully: {s}", .{container_id});
                }
            },
            else => {
                if (self.logger) |log| {
        ...
        if (self.logger) |log| {
            try log.info("Stop command completed successfully", .{});
        }
    }
};
