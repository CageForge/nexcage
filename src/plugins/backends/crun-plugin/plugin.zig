/// Crun Backend Plugin for NexCage
/// 
/// This plugin wraps the existing crun backend implementation as a plugin,
/// providing OCI container runtime functionality through the plugin system.

const std = @import("std");
const plugin = @import("../../../plugin/mod.zig");
const crun_backend = @import("../../../backends/crun/mod.zig");
...
export const metadata = plugin.PluginMetadata{
    ...
    .description = "Crun OCI container runtime backend plugin",
    .author = "NexCage Team",
    .api_version = 1,
...
const backend_extension = plugin.BackendExtension{
    .name = "crun",
    .description = "Crun OCI container runtime",
    .version = plugin.SemanticVersion{ .major = 1, .minor = 0, .patch = 0 },
...
/// Plugin state
var crun_driver: ?crun_backend.CrunDriver = null;
...
fn pluginInit(context: *plugin.PluginContext) !void {
    ...
    // Initialize the crun driver
    crun_driver = crun_backend.CrunDriver.init(context.allocator, null);
    
    std.log.info("Crun backend plugin '{}' initialized", .{context.getPluginName()});
}

/// Plugin cleanup
fn pluginDeinit(context: *plugin.PluginContext) void {
    if (crun_driver) |*driver| {
        driver.deinit();
    }
    crun_driver = null;
    plugin_allocator = null;
    
    std.log.info("Crun backend plugin '{}' deinitialized", .{context.getPluginName()});
}
...
fn backendCreate(context: *plugin.PluginContext, container_id: []const u8) !void {
    const driver = crun_driver orelse return error.DriverNotInitialized;
    
    // Convert container_id to SandboxConfig
    // For now, create a basic config - in real implementation this would come from container spec
    ...
    std.log.info("Container '{}' created via crun plugin from context '{}'", 
...
fn backendStart(context: *plugin.PluginContext, container_id: []const u8) !void {
    ...
    std.log.info("Container '{}' started via crun plugin from context '{}'", 
        .{ container_id, context.getPluginName() });
}
...
fn backendStop(context: *plugin.PluginContext, container_id: []const u8) !void {
    ...
    std.log.info("Container '{}' stopped via crun plugin from context '{}'", 
        .{ container_id, context.getPluginName() });
}
...
fn backendDelete(context: *plugin.PluginContext, container_id: []const u8) !void {
    ...
    std.log.info("Container '{}' deleted via crun plugin from context '{}'", 
        .{ container_id, context.getPluginName() });
}
...
fn backendList(context: *plugin.PluginContext, allocator: std.mem.Allocator) ![]const u8 {
    _ = context;
    _ = crun_driver orelse return error.DriverNotInitialized;
    
    // For now, return a basic JSON list
    // In real implementation, this would call crun list and parse the output
...
fn backendInfo(context: *plugin.PluginContext, container_id: []const u8, allocator: std.mem.Allocator) ![]const u8 {
    ...
    // In real implementation, this would call crun state and parse the output
    const result = try std.fmt.allocPrint(allocator,
        \\{{"id": "{s}", "status": "unknown", "runtime": "crun"}}
    , .{container_id});
...
test "crun plugin metadata validation" {
    try testing.expect(metadata.validate());
    try testing.expect(std.mem.eql(u8, metadata.name, "crun-backend"));
    try testing.expect(metadata.provides_backend);
}

test "crun plugin capabilities" {
...
test "crun plugin health check mock" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();
    
    // Mock context
    const context = try plugin.PluginContext.init(allocator, "test-crun-plugin");
    defer context.deinit();
    
    // Set allocator for health check
    plugin_allocator = allocator;
    defer plugin_allocator = null;
    
    // Health check should handle missing crun binary gracefully
    const health = pluginHealthCheck(context) catch plugin.HealthStatus.unhealthy;
    try testing.expect(health == .healthy or health == .degraded or health == .unhealthy);
}