...
pub const CommandInterface = struct {
    ...
    /// Opaque context pointer to concrete command instance
    ctx: *anyopaque,

    /// Execute command
    execute: *const fn (self: *CommandInterface, options: types.RuntimeOptions, allocator: std.mem.Allocator) Error!void,

    /// Get command help
    help: *const fn (self: *CommandInterface, allocator: std.mem.Allocator) Error![]const u8,

    /// Validate command arguments
    validate: *const fn (self: *CommandInterface, args: []const []const u8) Error!void,
};
