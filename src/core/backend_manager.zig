...
const logger_mod = @import("logging.zig");
const plugin = @import("oci/backend/plugin.zig");
...
pub const BackendManager = struct {
    ...
    logger: *logger_mod.LogContext,

    pub fn init(allocator: Allocator, logger: *logger_mod.LogContext) !Self {
    ...
    pub fn initializePlugins(self: *Self) !void {
        ...
        var crun_plugin = try backends_mod.crun.createCrunPlugin(self.allocator, self.logger);
        try self.registry.register(&crun_plugin);
        try self.logger.info("Registered Crun backend plugin", .{});

        // Register Proxmox LXC backend
        var proxmox_lxc_plugin = try backends_mod.proxmox_lxc.createProxmoxLXCPlugin(self.allocator, self.logger);
        ...
        var proxmox_vm_plugin = try backends_mod.proxmox_vm.createProxmoxVMPlugin(self.allocator, self.logger);
    ...
    pub fn listAvailableBackends(self: *Self) ![]plugin.BackendType {
        ...
        const backend_list = try self.registry.listBackends();
        defer self.allocator.free(backend_list);

        for (backend_list) |backend_type| {
