...
test "createTemplateFromRootfs negative case - upload fails with ConnectionResetByPeer, retries exhausted" {
    ...
    try std.testing.expectError(error.TemplateCreationFailed, template_ops.createTemplateFromRootfsWithDeps(&mock_client, rootfs_path, "test-template", deps));
}
...
test "createTemplateFromRootfs negative case - tar archive creation fails" {
    ...
    try std.testing.expectError(error.TemplateCreationFailed, template_ops.createTemplateFromRootfsWithDeps(&mock_client, "/non/existent/path", "test-template", deps));
}
