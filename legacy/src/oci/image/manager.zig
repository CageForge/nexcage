...
pub const ImageManager = struct {
    ...
    pub fn createContainerFromImage(
        ...
        try self.logger.info("Creating container {s} from image {s}:{s}", .{ container_id, image_name, image_tag });
        ...
        try self.logger.info("Container {s} created successfully from image {s}:{s}", .{ container_id, image_name, image_tag });
    }
    ...
    fn validateImageManifest(self: *Self, image_name: []const u8, image_tag: []const u8) !void {
        const manifest_path = try std.fs.path.join(self.allocator, &[_][]const u8{ self.images_dir, image_name, image_tag, "manifest.json" });
    ...
    fn checkImageConfiguration(self: *Self, image_name: []const u8, image_tag: []const u8) !void {
        const config_path = try std.fs.path.join(self.allocator, &[_][]const u8{ self.images_dir, image_name, image_tag, "config.json" });
    ...
    fn verifyLayerIntegrity(self: *Self, image_name: []const u8, image_tag: []const u8) !void {
        ...
        const layers_dir = try std.fs.path.join(self.allocator, &[_][]const u8{ self.images_dir, image_name, image_tag, "layers" });
        ...
        while (try iter.next()) |entry| {
            if (entry.kind == .file) {
                const layer_path = try std.fs.path.join(self.allocator, &[_][]const u8{ layers_dir, entry.name });
    ...
    fn setupLayerFSForContainer(self: *Self, container_id: []const u8, bundle_path: []const u8) !void {
        if (self.layer_fs) |_| {
            ...
            const container_mount = try std.fmt.allocPrint(self.allocator, "{s}/mounts/{s}", .{ bundle_path, container_id });
        ...
        if (self.layer_fs) |layerfs| {
            ...
            while (try iter.next()) |entry| {
                if (entry.kind == .file) {
                    ...
                    const layer = try Layer.createLayer(self.allocator, "application/vnd.oci.image.layer.v1.tar", entry.name, try self.getFileSize(layer_path), null);
    ...
    fn createContainerFilesystem(self: *Self, container_id: []const u8, bundle_path: []const u8) !void {
        ...
        const rootfs_path = try std.fs.path.join(self.allocator, &[_][]const u8{ bundle_path, container_id, "rootfs" });
    ...
    fn setupContainerMetadata(self: *Self, container_id: []const u8, image_name: []const u8, image_tag: []const u8) !void {
        ...
        const metadata_path = try std.fs.path.join(self.allocator, &[_][]const u8{ self.images_dir, "containers", container_id });
        defer self.allocator.free(metadata_path);

        try std.fs.cwd().makePath(metadata_path);

        // Create container info file
        const info_path = try std.fs.path.join(self.allocator, &[_][]const u8{ metadata_path, "info.json" });
        defer self.allocator.free(info_path);

        const info_file = try std.fs.cwd().createFile(info_path, .{});
        defer info_file.close();

        const info_content = try std.fmt.allocPrint(self.allocator, "{{\"container_id\":\"{s}\",\"image\":\"{s}:{s}\",\"created\":\"{s}\"}}", .{ container_id, image_name, image_tag, std.time.timestamp() });
