...
pub const BFCContainer = struct {
    ...
    pub fn extractFile(self: *Self, file_path: []const u8, output_path: []const u8) !void {
        ...
        const result = c.bfc_extract_to_file(self.container, file_path.ptr, output_path.ptr);
    ...
    pub fn setEncryptionPassword(self: *Self, password: []const u8) !void {
        ...
        const result = c.bfc_set_encryption_password(self.container, password.ptr, password.len);
...
pub const BFCBuilder = struct {
    ...
    pub fn addFile(self: *Self, file_path: []const u8, data: []const u8, mode: u32) !void {
        ...
        const result = c.bfc_add_file(self.writer, file_path.ptr, data.ptr, data.len, mode, std.time.nanoTimestamp());
    ...
    pub fn addDir(self: *Self, dir_path: []const u8, mode: u32) !void {
        ...
        const result = c.bfc_add_dir(self.writer, dir_path.ptr, mode, std.time.nanoTimestamp());
    ...
    pub fn setEncryption(self: *Self, password: []const u8) !void {
        ...
        const result = c.bfc_set_encryption_password(self.writer, password.ptr, password.len);
