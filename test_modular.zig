...
pub fn main() !void {
    ...
    // Test logger
    var logger = core.LogContext.init(allocator, std.io.getStdOut().writer(), config.log_level, "test");
    defer logger.deinit();

    try logger.info("Logger initialized successfully", .{});
    try logger.debug("Debug message test", .{});
    try logger.warn("Warning message test", .{});
    try logger.err("Error message test", .{});

    std.debug.print("✓ All core modules working correctly!\n", .{});
}
